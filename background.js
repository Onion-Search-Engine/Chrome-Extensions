...
function resetDefaultSuggestion() {
  chrome.omnibox.setDefaultSuggestion({
    description: '<url><match>src:</match></url> Onion Search Engine'
  });
}
...
function updateDefaultSuggestion(text) {
  ...
  var description = '<match><url>Onion Search Engine</url></match><dim> [</dim>';
  description +=
...
function search(query, callback) {
  ...
  var url = "https://www.onionsearchengine.com/search.php?search=" + query +
      "&submit=Search&rt=std&pk_campaign=chrome";
...
function getUrl(path, line) {
  var url = "https://www.onionsearchengine.com/search.php?search=" + path
      "&submit=Search&rt=std&pk_campaign=chrome";
  if (line)
    url += "&l=" + line;
  return url;
...
chrome.omnibox.onInputEntered.addListener(function(text) {
  ...
  if (/@\d+\b/.test(text)) {
    ...
    navigate("https://www.onionsearchengine.com/search.php?search=" +
              text +
             "&submit=Search&rt=std&pk_campaign=chrome");
  }
});
